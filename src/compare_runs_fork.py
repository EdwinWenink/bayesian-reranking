...
def plot(all_results, metric, stats, N, strategy, ymin=-1, ymax=1, output_path="../results/eval/"):
    ...
    ax.grid(True)
    ax.set_title(f"Per-topic analysis on {metric} (t={stats[0]:.6}, p={stats[1]:.6}) ")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--strategy", type=str, help='used strategy', default='GREEDY')
    parser.add_argument("--N", type=str, help='max value of the y axis', default='N100')
    ...
    os.system(f'trec_eval -q -M1000 -m {metric} {qrels} {base} > ../results/eval.base')
    os.system(f'trec_eval -q -M1000 -m {metric} {qrels} {comp} > ../results/eval.comp')

    base_metrics = load_metrics('../results/eval.base')
    comp_metrics = load_metrics('../results/eval.comp')
    ...
    plot(all_results, metric, stats, N, strategy, ymin=args.ymin, ymax=args.ymax)
